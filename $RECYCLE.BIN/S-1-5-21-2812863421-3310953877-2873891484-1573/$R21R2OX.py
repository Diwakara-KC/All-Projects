...
cur = conn.cursor()
cur.execute('''CREATE TABLE HOTEL_DB1
...
def send():
    if textReceipt.get(1.0, END) == '\n':
        ...
        if costofothersvar.get() != '0 Rs':
            textarea.insert(END, f'Cost Of Others\t\t\t{priceofOthers}Rs\n')

        textarea.insert(END, f'Sub Total\t\t\t{subtotalofItems}Rs\n')
        textarea.insert(END, f'Service Tax\t\t\t{0}Rs\n')
        textarea.insert(END, f'Total Cost\t\t\t{subtotalofItems}Rs\n')
...
def receipt():
    ...
    if costoffoodvar.get() != '' or costofothersvar.get() != '' or costofdrinksvar.get() != '':
        ...
        cur.execute("INSERT INTO HOTEL_DB1 (ID,Today_Date,Name,Phone_Number,Price) \
